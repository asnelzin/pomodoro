...
GOOS=linux GOARCH=amd64 CGO_ENABLED=0 go build -ldflags "-X main.revision=$REV" -o ../target/pomodoro

cd ..
docker build -t asnelzin/pomodoro:${BRANCH} -f Dockerfile.build .
docker push asnelzin/pomodoro:${BRANCH}